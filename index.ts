...
class invite {
  ...
  public accept = async (invite: Invite, cb: any) => {
    ...
    if (e0) return cb(clarify(e0, 'Could not accept invalid invite code'));
    ...
    if (e1) return cb(clarify(e1, 'Could not connect to pub'));

    // command the peer to follow me
    const [e2] = await run(rpc.invite.use)({feed: this.ssb.id});
    if (e2) return cb(clarify(e2, 'Invite not accepted by the pub'));
    ...
    if (shouldPublish) {
      ...
      if (e3) return cb(clarify(e3, 'Unable to follow friend behind invite'));
      ...
      if (e4) return cb(clarify(e4, 'Unable to announce pub to my friends'));
    }

    // remember in SSB CONN
    const [e5] = await run(this.ssb.conn.remember)(addr, connData);
    if (e5) return cb(clarify(e5, 'Could not store the pub in ssb-conn'));
